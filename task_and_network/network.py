...
class Model(object):
    ...
    def _build(self, hp):
        ...
        self.saver = tf.train.Saver(max_to_keep=0)##############modified by yichen(add max_to_keep=0 to keep all the checkpoints) 
    ...
    def save(self,modeldir=None):#modified by yichen
        """Save the model."""
        if modeldir == None: #add by yichen
            modeldir = self.model_dir #add by yichen
        sess = tf.get_default_session()
        ...
        for v in self.var_list:
            if 'kernel' in v.name or 'weight' in v.name:
                ...
                if 'output' in v.name:
                    ...
                    # recurrent weights
                    v_val[n_input + units, :] = 0 #fused input
                    #v_val[units, :] = 0 #separate input add by yichen
